...
| Ty_Lam : forall g x a body b eps beta,
    ...
    has_type g (TLam x a body) (TyArrow a b) EffEmpty (BFinite 0)
| Ty_App : forall g f a arg_ty ret_ty epsf epsa betaf betaa,
    has_type g f (TyArrow arg_ty ret_ty) epsf betaf ->
    has_type g a arg_ty epsa betaa ->
    has_type g (TApp f a) ret_ty (eff_join epsf epsa) (bound_seq betaf betaa)
...
| Ty_FixStructural : forall g f x body eps beta,
    ...
    has_type ((x, TyNat) :: (f, TyArrow TyNat TyNat) :: g) body TyNat eps beta ->
    has_type g (TFix f x TyNat body Structural) (TyArrow TyNat TyNat) EffEmpty beta
| Ty_FixMeasure : forall g f x body eps beta,
    (* SPEC-GAP(measure-tag-trusted-reduced-core): reduced core trusts the measure tag. *)
    has_type ((x, TyNat) :: (f, TyArrow TyNat TyNat) :: g) body TyNat eps beta ->
    has_type g (TFix f x TyNat body Measure) (TyArrow TyNat TyNat) EffEmpty beta
| Ty_FixDiv : forall g f x body eps beta,
    has_type ((x, TyNat) :: (f, TyArrow TyNat TyNat) :: g) body TyNat eps beta ->
    has_type g (TFix f x TyNat body Div) (TyArrow TyNat TyNat) EffEmpty BOmega
