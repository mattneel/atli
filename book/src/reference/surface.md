...
The implemented surface is specified in [`docs/syntax.md`](../../docs/syntax.md). The Book narrates; `docs/` specifies. v0.5.3 includes `Nat`, `Unit`, `Array`, records, variants, `^` uniqueness, `move`/`inplace`/`freeze`, functions, `case`, `measure`/`div`, multi-label effects, handlers, arithmetic prelude operators, mutual top-level recursion, `scope`/`spawn`/`await`, type parameters, generic `Array[A]`/records/variants/functions, and `^u` preservation.
