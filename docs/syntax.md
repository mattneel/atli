...
> **Status: implemented subset + open list (v0.5.1).** This document began as draft-0.
> The v0.5.1 compiler implements the reduced surface listed below and keeps the rest in
...
Unsupported v0.5.1 constructs diagnose as "not yet in the reduced surface".
...
type Option[A] = None | Some(A)   // generic variants are implemented in v0.5.1
...
binding in v0.5.1: after a shared use there is no remaining unique handle to mutate. Write
...
closures are not spawnable in v0.5.1. `await h` consumes an opaque affine task handle and
...
- **Effect-row variables/open rows.** The blocked target is `map[A, B](xs: List[A], f: A -> B ! e) -> List[B] ! e`; v0.5.1 generic higher-order arguments are pure. Syntax `! e` / `! {L | e}` remains open until row unification lands.
